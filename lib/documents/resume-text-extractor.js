...
function pythonPathEnv() {
  const entries = [join(process.cwd(), ".python-packages")];
...
function dependencyFailureFor({ kind, lastError }) {
  const errorText = String(lastError?.error || "");
  if (kind === "pdf" && /No module named ['"]?pypdf['"]?/i.test(errorText)) {
    return {
      code: "PDF_TEXT_EXTRACTOR_DEPENDENCY_MISSING",
      message: "云端 PDF 文本解析依赖 pypdf 未安装，Render 需要重新部署最新代码以安装 requirements.txt 后再解析 PDF。"
    };
  }
  return null;
}
...
async function runPythonExtractor({ buffer, kind, fileName }) {
  ...
  const dependencyFailure = dependencyFailureFor({ kind, lastError });
